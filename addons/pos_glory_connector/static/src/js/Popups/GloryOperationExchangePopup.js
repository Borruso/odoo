odoo.define("pos_glory_connector.GloryOperationExchangePopup", function (require) {
    "use strict";
    ...
    class GloryOperationExchangePopup extends AbstractAwaitablePopup {
        setup() {
            super.setup();
            ...
            xmlhttp.onreadystatechange = async function () {
                if (xmlhttp.readyState === 4) {
                    ...
                    if (xmlhttp.status === 200) {
                        ...
                        const result = cashoutResponse[0].attributes["n:result"].value;
                ...
                if (xmlhttp.readyState === 4) {
                    ...
                    if (xmlhttp.status === 200) {
                        ...
                        if (result === "0" || result === "11") {
                            ...
                            self.state.exchangeAmount =
                                self.env.pos.round_decimals_currency(moneyCash);
                            self.state.sendEndCashinRequest = true;
                            self.state.showEndCashinRequest = false;
                            self.env.pos.exchangeAmount = self.state.exchangeAmount;
                            await new Promise((resolve) => setTimeout(resolve, 3000));
                            const {confirmed, payload} = await self.showPopup(
                                "GloryCashoutPopup"
                            );
                            if (!confirmed) return;
                            const {moneyDetails, total} = payload;
                            self.env.pos.exchangeAmount = 0;
                            var convertTotal = self.env.pos.round_decimals_currency(
                                -1 * total
                            );
                            self.state.exchangeAmount = convertTotal;
                            self.state.moneyDetailsOut = moneyDetails;
                            self.state.sendCashoutRequest = true;
                            status(true);
                            const message = self.env._t(
    ...
    GloryOperationExchangePopup.template = "GloryOperationExchangePopup";
    GloryOperationExchangePopup.defaultProps = {cancelText: _lt("Cancel")};
    Registries.Component.add(GloryOperationExchangePopup);
    return GloryOperationExchangePopup;
});
